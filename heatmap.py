...
class heatmaps:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_box, renormalize):
        ...
        # print(model.model[9].token_mixer.excite)
        ...
        target_layers = []
        for l in layer:
            target_layers.append(get_layer_by_path(model, l))  # 使用 get_layer_by_path 动态访问
    ...
    def __call__(self, img_path, save_path, name):
        # remove dir if exist
        ...
        if os.path.isdir(img_path):
            ...
            self.process(img_path, f'{save_path}/{name}')
...
def get_layer_by_path(model, path):
    """
    通过路径（如 model.model[9].token_mixer.excite）获取模型中的特定层。

    :param model: PyTorch 模型
    :param path: 层的路径，字符串类型，例如 'model.model[9].token_mixer.excite'
    :return: 对应层的对象
    """
    for part in path.split('.'):
        # 如果是索引形式 (例如 'model[9]')，则进行处理
        if part.startswith('model['):
            index = int(part[6:-1])  # 提取数字索引
            model = model[index]  # 根据索引访问模型中的某一层
        else:
            model = getattr(model, part)  # 直接通过名称访问子模块
    return model
...
def get_params():
    params = {
        ...
        'layer': ['model.model[9].token_mixer.pool_h'],
        'device': 'cuda:0',
...
if __name__ == '__main__':
    ...
    DetectionModel(cfg='yaml', nc=4) #cfg='模型的yaml文件'，nc为类别数字
  # 查看模型结构
    model = YOLO('yaml')
    print(model.info)


    model = heatmaps(**get_params())
    # jpg为对应照片路径或者文件夹路径，savedir为保存的文件夹路径，savename为保存的名字（.jpg结尾）
    model('jpg', 'savedir', 'savename') 
